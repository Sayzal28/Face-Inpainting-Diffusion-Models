...
from PIL import Image  # FIXED: Move import to top
import lpips
...
class MetricsCalculator:
    ...
    def add_batch(self, original_images, inpainted_images):
        ...
        # FIXED: Keep in [-1, 1] range and store on CPU immediately
    ...
    def calculate_lpips_batch(self, img1, img2):
        """FIXED: Calculate LPIPS for a batch - expects [-1, 1] range."""
        with torch.no_grad():
            # LPIPS expects [-1, 1] range - no conversion needed
            lpips_dist = self.lpips_fn(img1, img2)
            return lpips_dist.cpu().numpy()  # Return per-image scores
    
    def calculate_ssim_batch(self, img1, img2):
        """FIXED: Calculate SSIM for a batch with proper parameters."""
        ...
        for i in range(img1_np.shape[0]):
            ...
            # FIXED: Use proper SSIM parameters
            score = ssim(
                img1_hwc, 
                img2_hwc, 
                channel_axis=2,  # FIXED: Use channel_axis instead of multichannel
                data_range=1.0,  # Correct range for [0,1] data
    ...
    def save_images_for_fid(self, output_dir):
        ...
        if not FID_AVAILABLE or not self.all_original:
            return None, None
        ...
        # FIXED: Convert to [0,1] range for PIL
        all_real = (all_real + 1) / 2
        all_fake = (all_fake + 1) / 2
        
        # Save images
        for i, (real_img, fake_img) in enumerate(zip(all_real, all_fake)):
            try:
                # Convert to PIL format
                real_pil = transforms.ToPILImage()(real_img.clamp(0, 1))
    ...
    def calculate_fid(self, real_dir, fake_dir):
        """FIXED: Calculate FID score with proper error handling."""
        if not FID_AVAILABLE:
            return None
            
        try:
            print("Calculating FID score...")
            # FIXED: Use larger batch size and proper error handling
            fid_value = fid_score.calculate_fid_given_paths(
                [real_dir, fake_dir],
                batch_size=min(100, len(list(Path(real_dir).glob("*.png")))),  # Adaptive batch size
                device=self.device,
                dims=2048,
                num_workers=4
            )
            ...
            if "out of memory" in str(e):
                print("GPU memory error, trying with CPU...")
                try:
                    fid_value = fid_score.calculate_fid_given_paths(
                        [real_dir, fake_dir],
                        batch_size=50,
                    ...
                    print(f"CPU FID calculation also failed: {e2}")
                    return None
            else:
    ...
    def calculate_all_metrics(self, output_dir):
        """FIXED: Calculate all metrics with proper implementations."""
        ...
        # FIXED: Calculate LPIPS properly
...
class InpaintingSampler:
    ...
    def inpainting_ddim_sample_loop(self, model_fn, shape, gt_images, masks,
        ...
        for step_idx, timestep in timestep_iterator:
            ...
            with torch.no_grad():
                ...
                if model_output.shape[1] == 6:  # Model outputs both noise and variance
                    noise_pred = model_output[:, :3]  # First 3 channels are noise
                    # We ignore the variance prediction for DDIM
                elif model_output.shape[1] == 3:  # Model outputs only noise
                ...
                if timestep > 0:  # Don't inject at final step
    ...
    def run_sampling(self):
        ...
        for batch_idx, batch in enumerate(tqdm(self.dataloader, desc="Processing batches")):
            ...
            # FIXED: Add to metrics calculator immediately to save GPU memory
...
def parse_args():
    ...
    parser.add_argument('--ddim_timesteps', type=int, default=100,
    ...
    parser.add_argument('--ddim_eta', type=float, default=0.9,
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='inpainting_results_ddim',
...
def main():
    ...
    if args.quick_test:
        ...
        args.max_batches = 2
        args.batch_size = 4
        args.ddim_timesteps = 10  # Very few steps for quick test
